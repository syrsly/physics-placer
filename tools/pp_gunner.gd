...
class_name PPGunner
extends PPTool
## This is an early version of the Shoot tool. It may be non-functional. I keep this file in the project for easy reference.
...
func spawn_name_prefix() -> String:
	return settings.gun_name_prefix


func spawn_freeze() -> bool:
	return settings.gun_freeze_result
...
func activate() -> void:
	...
	_recoil = 0.0
	_rounds = 0
	if settings.gun_rapid_fire:
		set_status("Gunner: hold the left mouse button to fire (%.0f rounds/sec). Release to let them settle." % settings.gun_rate)
	else:
		set_status("Gunner: click to fire one round.")
...
func aim_point(camera: Camera3D) -> Vector2:
	if not settings.gun_fixed_crosshair and _screen_point.x >= 0.0:
...
func muzzle_point(camera: Camera3D) -> Vector3:
	...
	return camera.project_ray_origin(at) + aim_direction(camera) * settings.gun_muzzle_offset
...
func _spread_direction(camera: Camera3D, base: Vector3) -> Vector3:
	var half: float = deg_to_rad(clampf(settings.gun_spread, 0.0, 60.0)) * 0.5
	if half <= 0.0001:
		return base
	var right: Vector3 = camera.global_transform.basis.x.normalized()
...
func _trigger_down(camera: Camera3D) -> int:
	if _settling:
		return EditorPlugin.AFTER_GUI_INPUT_STOP
	if settings.gun_scene == null:
		set_status("Gunner: load an ammo object (PackedScene) first.")
		return EditorPlugin.AFTER_GUI_INPUT_STOP
	if camera == null:
		camera = editor_camera()
	if camera == null:
		set_status("Gunner: no 3D viewport camera found.")
		return EditorPlugin.AFTER_GUI_INPUT_STOP
	_camera = camera
	if not _armed and not _begin_burst(camera):
		return EditorPlugin.AFTER_GUI_INPUT_STOP
	_fire_round(camera)
	_firing = settings.gun_rapid_fire
	_cooldown = 1.0 / maxf(0.5, settings.gun_rate)
	return EditorPlugin.AFTER_GUI_INPUT_STOP
...
func _trigger_up() -> int:
	if not _armed:
		return EditorPlugin.AFTER_GUI_INPUT_PASS
	_firing = false
	if settings.gun_hold_to_accumulate:
		set_status("Gunner: %d round(s) fired. Keep firing, or press Finish Volley to place them." % _rounds)
		return EditorPlugin.AFTER_GUI_INPUT_STOP
	finish_volley()
	return EditorPlugin.AFTER_GUI_INPUT_STOP
...
func _begin_burst(camera: Camera3D) -> bool:
	...
	if root == null:
		set_status("Gunner: no 3D scene is open.")
		return false
	sim.reset()
	sim.setup(settings)
	sim.set_scene(root)
...
func _fire_round(camera: Camera3D) -> void:
	if _rounds >= settings.gun_max_rounds:
		_firing = false
		set_status("Gunner: magazine limit of %d reached. Release to place them." % settings.gun_max_rounds)
		return
	var root: Node = scene_root()
	if root == null:
		return
	...
	var speed: float = settings.gun_force * (1.0 + randf_range(-settings.gun_force_jitter, settings.gun_force_jitter))
	var rot := Basis.IDENTITY
	if settings.gun_random_rotation:
		rot = Basis.from_euler(Vector3(randf_range(0.0, TAU), randf_range(0.0, TAU), randf_range(0.0, TAU)))
	var spin := Vector3(
		randf_range(-settings.gun_spin, settings.gun_spin),
		randf_range(-settings.gun_spin, settings.gun_spin),
		randf_range(-settings.gun_spin, settings.gun_spin))
	var item = sim.add_spawn(settings.gun_scene, Transform3D(rot, muzzle_point(camera)), dir * speed, spin, 0.0, root)
	if item == null:
		set_status("Gunner: " + (sim.last_warning if sim.last_warning != "" else "the round could not be spawned."))
		_firing = false
		return
	_rounds += 1
	_recoil = minf(settings.gun_recoil_max, _recoil + settings.gun_recoil)
	set_status("Gunner: %d round(s) in the air." % _rounds)
...
func tick(delta: float) -> void:
	...
	if not _firing and _recoil > 0.0:
		_recoil = maxf(0.0, _recoil - settings.gun_recoil_recovery * delta)
	...
	if camera == null:
		return
	var interval: float = 1.0 / clampf(settings.gun_rate, 0.5, 60.0)
	_cooldown -= delta
	var guard := 0
...
func finish_volley() -> void:
	...
	_firing = false
	_settling = true
	set_status("Gunner: settling %d round(s)..." % _rounds)
...
func on_sim_finished() -> void:
	...
	_firing = false
	_settling = false
	finish_and_bake("Physics Placer: Gunfire", spawn_parent())
	_rounds = 0


func spawn_parent() -> Node:
	if not settings.gun_parent_selected:
		return scene_root()
...
func draw_overlay(overlay: Control, camera: Camera3D) -> void:
	...
	if settings.gun_spread > 0.01:
		var dist: float = _impact_distance(camera, muzzle, dir)
		var radius3d: float = tan(deg_to_rad(settings.gun_spread) * 0.5) * dist
	...
	if font != null:
		var mode: String = "auto %.0f/s" % settings.gun_rate if settings.gun_rapid_fire else "single"
		var label: String = "Gun  %.0f m/s   %s   rounds %d/%d" % [settings.gun_force, mode, _rounds, settings.gun_max_rounds]
...
func _draw_tracer(overlay: Control, camera: Camera3D, muzzle: Vector3, dir: Vector3) -> void:
	...
	var speed: float = maxf(0.01, settings.gun_force)
